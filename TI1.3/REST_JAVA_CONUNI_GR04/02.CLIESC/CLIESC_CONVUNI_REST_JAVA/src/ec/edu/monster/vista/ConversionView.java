...
public class ConversionView extends JFrame {
    ...
    private static final Locale UI_LOCALE = Locale.US;
    ...
    public void showResult(ConversionResult result) {
        lblResultadoPrincipal.setText(String.format(UI_LOCALE, "%.2f %s", result.output, result.outputUnit));
        lblResultado.setText(String.format(UI_LOCALE, "Entrada: %.2f %s", result.input, result.inputUnit));
    ...
    private void buildUI() {
        ...
        gridC.gridx = 0;
        gridC.gridy = 1;
        gridC.insets = new Insets(0, 0, 8, 0);
        root.add(grid, gridC);
        ...
        resC.gridx = 0;
        resC.gridy = 2;
        resC.insets = new Insets(6, 6, 6, 6);
        ...
        msgC.gridx = 0;
        msgC.gridy = 3;
        msgC.insets = new Insets(6, 80, 6, 80);
    ...
    private void convertir() {
        ...
        if (comboTipo.getSelectedItem() == null) {
            showValidationError("Seleccione un tipo de conversión.");
            return;
        }
    ...
    private static class FieldContainer extends JComponent {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2.fillRoundRect(3, 5, getWidth() - 6, getHeight() - 6, r, r);
            g2.setColor(Color.WHITE);
            g2.fillRoundRect(0, 0, getWidth() - 6, getHeight() - 6, r, r);
    ...
    private void agruparTiposPorCategoria() {
        ...
        for (String cat : orden) {
            tiposPorCategoria.put(cat, new ArrayList<>());
        }

        for (ConversionType t : ConversionType.values()) {
            String nombre = normaliza(t.toString());
            String cat = "Otros";
            if (contiene(nombre, "celsius", "fahrenheit", "kelvin", "temperatura", "centigrado")) {
                cat = "Temperatura";
            } else if (contiene(nombre, "metro", "centimetro", "kilometro", "milla", "yarda", "pie", "pulgada")) {
                cat = "Longitud";
            } else if (contiene(nombre, "gramo", "kilogramo", "libra", "onza", "masa")) {
                cat = "Masa";
            } else if (contiene(nombre, "litro", "mililitro", "galon", "volumen")) {
                cat = "Volumen";
            } else if (contiene(nombre, "metro cuadrado", "hectarea", "area")) {
                cat = "Área";
            } else if (contiene(nombre, "km/h", "kph", "mph", "velocidad")) {
                cat = "Velocidad";
            } else if (contiene(nombre, "segundo", "minuto", "hora", "tiempo", "dia")) {
                cat = "Tiempo";
            } else if (contiene(nombre, "bit", "byte", "kb", "mb", "gb", "tb", "datos")) {
                cat = "Datos";
            }
